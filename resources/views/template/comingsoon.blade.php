	...
	<script>
		function comingsooncounter() {
			...
			function setDate(){
				var now = new Date();
				if( launch < now ){
					days.blade.php('<p>Days</p><h1>0</h1>');
					hours.blade.php('<p>Hours</p><h1>0</h1>');
					minutes.blade.php('<p>Minutes</p><h1>0</h1>');
					seconds.blade.php('<p>Seconds</p><h1>0</h1>');
				}
				else{
					var s = -now.getTimezoneOffset()*60 + (launch.getTime() - now.getTime())/1000;
					var d = Math.floor(s/86400);
					days.blade.php('<h1>'+d+'</h1><p>Day'+(d>1?'s':''),'</p>');
					s -= d*86400;
					var h = Math.floor(s/3600);
					hours.blade.php('<h1>'+h+'</h1><p>Hour'+(h>1?'s':''),'</p>');
					s -= h*3600;
					var m = Math.floor(s/60);
					minutes.blade.php('<h1>'+m+'</h1><p>Minute'+(m>1?'s':''),'</p>');
					s = Math.floor(s-m*60);
					seconds.blade.php('<h1>'+s+'</h1><p>Second'+(s>1?'s':''),'</p>');
